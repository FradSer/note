...
// it. Evaluated before the row is written, so it sees the current table max; on a
// conflicting update it still exceeds the row's own seq.
...
const ENTITY_SQL: Record<string, EntitySQL> = {
  notes: entitySQL("notes"),
  note_folders: entitySQL("note_folders"),
};
...
app.get("/api/v1/:entity/pull", async (c) => {
  ...
  if (rawCursor === "") {
    ...
    if (/^\d+$/.test(seqPart)) {
      ...
      // over from before migration 0002. Restart from the beginning so the device
      // re-pulls and re-converges instead of stranding forever.
      cursorSeq = 0;
      cursorId = "";
    }
  }
